...
const DB = (() => {
  ...
  function fromApi(t) {
    return {
      ...t,
      id: String(t.id),
      tanggalMain: t.tanggal_main,
      jenisTransfer: t.jenis_transfer,
      tanggalTransfer: t.tanggal_transfer,
      jamMulai: t.jam_mulai ? t.jam_mulai.slice(0, 5) : "",
      jamSelesai: t.jam_selesai ? t.jam_selesai.slice(0, 5) : "",
      okupansiJam: Number(t.okupansi_jam) || 0,
      gambarBukti: t.gambar_bukti,
    };
  }
  ...
  function toApi(entry) {
    return {
      nama: entry.nama,
      tanggal_main: entry.tanggalMain,
      jenis_transfer: entry.jenisTransfer,
      tanggal_transfer: entry.tanggalTransfer,
      nominal: entry.nominal,
      catatan: entry.catatan || null,
      jam_mulai: entry.jamMulai || null,
      jam_selesai: entry.jamSelesai || null,
      okupansi_jam: entry.okupansiJam || 0,
      gambar_bukti: entry.gambarBukti || null,
    };
  }
  ...
  async function request(url, options = {}) {
    ...
    if (!response.ok) throw new Error(`API transaksi gagal (${response.status})`);
  ...
  return {
    getAll,
    async getById(id) {
      return fromApi(await request(`${endpoint}/${id}`));
    },
    async add(entry) {
      return fromApi(await request(endpoint, { method: "POST", body: JSON.stringify(toApi(entry)) }));
    },
    async update(id, entry) {
      await request(`${endpoint}/${id}`, { method: "PUT", body: JSON.stringify(toApi(entry)) });
      return true;
    },
    async remove(id) {
    ...
    async groupByMonth() {
      const map = {};
      (await getAll()).forEach((t) => {
        const key = monthKeyOf(t.tanggalTransfer);
        if (!key) return;
        if (!map[key]) map[key] = { key, count: 0, totalNominal: 0, totalOkupansi: 0 };
        map[key].count += 1;
        map[key].totalNominal += Number(t.nominal) || 0;
        map[key].totalOkupansi += Number(t.okupansiJam) || 0;
      });
    ...
    monthLabel(key) {
      ...
      return ["Jan", "Feb", "Mar", "Apr", "Mei", "Jun", "Jul", "Agu", "Sep", "Okt", "Nov", "Des"][month - 1] + ` ${year}`;
    },
    async summary(filterMonthKey) {
      let all = await getAll();
      if (filterMonthKey) all = all.filter((t) => monthKeyOf(t.tanggalTransfer) === filterMonthKey);
      const totalNominal = all.reduce((sum, t) => sum + (Number(t.nominal) || 0), 0);
      const totalOkupansi = all.reduce((sum, t) => sum + (Number(t.okupansiJam) || 0), 0);
      return { count: all.length, totalNominal, totalOkupansi, rataRata: all.length ? Math.round(totalNominal / all.length) : 0 };
    },
  };
})();
